...
# Print factor levels to verify encoding (in actual run, this output will be hidden)
cat("Charge levels (reference first):", levels(insurance_data$charges), "\n")
cat("Sex levels (reference first):", levels(insurance_data$sex), "\n")
cat("Smoker levels (reference first):", levels(insurance_data$smoker), "\n")
cat("Region levels (reference first):", levels(insurance_data$region), "\n")
...
- **Age** has strongest correlation with high charges (`r round(correlation_matrix["age", "charges_numeric"], 2)`)
- **Smoking status** has second strongest correlation (`r round(correlation_matrix["smoker_numeric", "charges_numeric"], 2)`)
...
# Print model summary for verification (hidden in final output)
...
# cat("First few values of y_train:", head(y_train), "\n")
# cat("Corresponding charges values:", head(as.character(train_data$charges[1:6])), "\n")
...
- Age as the dominant predictor with Gini importance of `r round(importance_df$MeanDecreaseGini[importance_df$Variable == "age"], 1)`
- Smoker status as second most important with importance of `r round(importance_df$MeanDecreaseGini[importance_df$Variable == "smoker"], 1)`
...
# Verify encoding consistency
# cat("Sample of train_y values:", head(train_y), "\n")
# cat("Corresponding charges values:", head(as.character(train_data$charges[1:6])), "\n")
...
# ------------------------------------------------
# Note: The 'cat' function messages are included but will be hidden by results='hide'
results <- data.frame()
...
The feature importance analysis again confirms smoking, age, and BMI as key predictors, with smoking status contributing a gain of `r round(xgb_importance$Gain[xgb_importance$Feature == "smokeryes"], 3)`, followed by age and BMI with gains of `r round(xgb_importance$Gain[xgb_importance$Feature == "age"], 3)` and `r round(xgb_importance$Gain[xgb_importance$Feature == "bmi"], 3)` respectively. Although here, we observe children as a more important predictor than BMI (the reverse of the Random Forest results).
...
roc_plot <- ggplot(roc_data, aes(x = FPR, y = TPR, color = ModelLabel)) +
  ...
  # Add points at specific thresholds for reference (optional)
  # geom_point(data = subset(roc_data, FPR > 0.1 & FPR < 0.15), size = 3, alpha = 0.8) +
  # Add labels
  labs(
  ...
  # Add shaded area under the best model's curve
  geom_ribbon(
    data = subset(roc_data, Model == best_auroc_model),
...
3. **AUROC**: `r best_auroc_model` tops the Area Under ROC Curve metric at `r round(best_auroc_score, 3)`
...
Based on our comprehensive evaluation, `r best_f1_model` is selected as the optimal model for the following reasons:
...
4. **Consistent Feature Importance**: Smoking status, age, and BMI are consistently ranked as top predictors across all models, and `r best_f1_model` appropriately weights these factors
...
prediction_summary <- data.frame(
  ...
  setNames(c("Class", "Percentage", "Count"))
...
1. **Model Performance**: `r best_f1_model` achieved the highest F1 score (`r round(best_f1_score, 3)`), though different models excelled in different metrics. The strong performance of tree-based models confirms the presence of complex, non-linear relationships in insurance cost factors.
...
The `r best_f1_model` model's strong performance on the F1 score metric ensures reliable classification even with class imbalance, providing a balanced approach to identifying both high and low charge cases.
...
The code and data for this analysis are available in the GitHub repository: [sl-assignment2](https://github.com/ces0491/sl-assignment2.git)
